...
class JamfPro:
    class JamfUAPIAuthToken(object):
        ...
        @property
        def token(self):
            print(f"{self._token_expires - 15.0} > {self.unix_timestamp()}\n {self._token_expires - self.unix_timestamp()}")
        ...
        def get_token(self):
            ...
            if response.status_code != 200:
                raise Exception
            dtg = datetime.strptime(response.json()['expires'], "%Y-%m-%dT%H:%M:%S.%fZ")
    ...
    def getHeaders(self):
        ...
        headers['User-Agent'] = 'SplunkBase_TA-Jamf-AddOn/2.10.4 modular/jamfComputers'
        return headers
    ...
    def _url_get_call(self, URL=""):
        ...
        :return:
        """
        print(json.dumps(self.getHeaders))
        url = URL
        response_dict = None
        for i in range(0, 3):
            try:
